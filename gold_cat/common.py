...
data_path = os.path.join(cur_dir, 'full-simplified-cat.ndjson')
...
def create_varied_del_instance(drawing):
    ...
    sample_size = random.randint(1, len(lines)-1)
...
def create_bad_varied_instance(drawing):
    drawing = randomize(drawing, step_max = 5, wrong_step_min = 10, wrong_step_max=20, wrong_max_perm=10)
    return create_instance(drawing)
...
def create_batch1(batch_size, gold, neg_samples, mixed_missing=False):
    ...
    for i in range(neg):
        xs.append(normalize(create_varied_instance(neg_samples[random.randint(0, len(neg_samples)-1)])))
        ys.append([1,0])
...
def randomize(strokes, step_max = 5, clip_min = 1, clip_max = 255,wrong_step_min = None, wrong_step_max = None, wrong_max_perm = 1000):
    ret = []
...
def main():
    ...
    total_num = 0
    under150 = 0
    with jsonlines.open('/Users/xin/Downloads/full-simplified-cat.ndjson') as reader:
        for (idx, obj) in enumerate(reader):
            strokes = obj['drawing']
